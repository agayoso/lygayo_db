...
SELECT formulario,
       v01b AS nro_dormitorios,
       v03 AS tipo_piso,
       v04 AS tipo_techo,
       v12b AS tipo_combustible,
       v1604 AS tiene_termocalefon,
       v1609 AS tiene_auto_camion,
       v1605 AS tiene_acondicionador_aire,
       v1607 AS tiene_horno_microondas,
       v1606 AS tiene_tv_cable,
       v15b AS tiene_internet,
       v1603 AS tiene_cocina_elec,
       v15a1 AS tiene_computadora,
       v15a2 AS tiene_televisor,
       p09 AS edad,
       p04 AS relacion_parentesco,
       p05 AS es_miembro_hogar,
       p11 AS estado_civil,
       s01 AS tiene_seguro,
       a02 AS trabajo_7dias,
       a03 AS trabajo_1hr,
       a04 AS trabajo_no_realizado,
       b06 AS tipo_empleo,
       ed01 AS idioma,
       añoest,
       totpers,
...
INTO #PMT_URBANO
FROM RSH_BASE
WHERE dominio = 1 AND p05 = 1 AND p04 < 11
GO

-- Agregar columnas para categorías de vivienda
...
-- Crear tabla temporal para jefe
...
-- Crear tabla temporal para hogar
...
SELECT pu.formulario, pu.area, pu.dpto, pu.dominio,
       ...
       pu.tiene_termocalefon, pu.tiene_auto_camion, pu.tiene_acondicionador_aire, pu.tiene_horno_microondas,
       pu.tiene_tv_cable, pu.tiene_internet, pu.tiene_cocina_elec, pu.tiene_computadora, pu.tiene_televisor
INTO #tabla_final
FROM #PMT_URBANO pu
...
-- Agregar columnas para ingreso estimado y estatus de pobreza
...
-- Declarar constantes y ponderaciones
...
DECLARE @w_tiene_cocina_elec FLOAT = 0.0424315
DECLARE @w_tiene_computadora FLOAT = 0.1464662
DECLARE @w_tiene_televisor FLOAT = 0.0933939
...
SET lninc_est = @constante +
    ...
    tiene_cocina_elec*@w_tiene_cocina_elec +
    tiene_computadora*@w_tiene_computadora +
    tiene_televisor*@w_tiene_televisor
GO

-- Calcular estatus de pobreza
UPDATE #tabla_final
